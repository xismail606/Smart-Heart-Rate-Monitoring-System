  ...
  options: {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        borderColor: '#ff2d55', borderWidth: 1,
        titleColor: '#ff2d55', bodyColor: '#e8e8f0',
...
let allBPMs = [];
let hasData = false, lastBPM = null;
...
function startSessionTimer() {
  ...
  document.getElementById('sessionStart').textContent = formatTime(new Date(sessionStart));
...
function updateDashboard(bpm, time) {
  ...
  if      (bpm < 60)   { s.textContent = 'Low';       s.className = 'stat-value caution'; }
  else if (bpm <= 100)  { s.textContent = 'Normal ✓';  s.className = 'stat-value normal';  }
  else if (bpm <= 140)  { s.textContent = 'High ⚠';    s.className = 'stat-value high';    }
  else                   { s.textContent = 'Danger 🚨'; s.className = 'stat-value high';     }

  // Max / Min
  if (bpm > maxBPM) { maxBPM = bpm; document.getElementById('maxValue').textContent = maxBPM; }
  if (bpm < minBPM) { minBPM = bpm; document.getElementById('minValue').textContent = minBPM; }
